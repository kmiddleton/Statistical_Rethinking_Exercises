...
beta <- function(x, w, n) dbeta(x, w + 1, n - w + 1)
norm <- function(x, w, n){
  dt <- list(w = w, n = n)
  flist <- alist(
    w ~ dbinom(n, p),
  ...
  res <- rethinking::map(flist, dt)
  res <- precis(res)@output
  
  dnorm(x, res$Mean, res$StdDev)
}

dt <- data.frame(x = seq(from = 0, to = 1, by = .01))

qa_graph <- function(dt, w, n){
  ggplot(dt, aes(x = x)) + stat_function(fun = beta, geom = "line", args = list(w = w, n = n), color = "blue") + stat_function(fun = norm, geom = "line", args = list(w = w, n = n)) + ylab("Density") + xlab("proportion water") + ggtitle(paste0("n = ", n))
}
